...
class WPB_PCF_Review_Notice
{
	...
	const META_LATER          = 'wpb_pcf_review_later';
	const REVIEW_URL          = 'https://wordpress.org/support/plugin/wpb-popup-for-contact-form-7/reviews/?rate=5#new-post';
	const NONCE_ACTION        = 'wpb_pcf_review_notice_action';
	...
	private function render_notice()
	{
		...
		$rate_url    = esc_url(add_query_arg(array('wpb_pcf_review_action' => 'rate',    '_wpnonce' => $nonce)));
		$later_url   = esc_url(add_query_arg(array('wpb_pcf_review_action' => 'later',   '_wpnonce' => $nonce)));
		$dismiss_url = esc_url(add_query_arg(array('wpb_pcf_review_action' => 'dismiss', '_wpnonce' => $nonce)));
?>
		<div class="wpb-pcf-review-notice notice" style="border-left: 4px solid #f0b429; padding: 0; margin: 15px 0; border-radius: 3px; box-shadow: 0 1px 3px rgba(0,0,0,.06);">
			<div style="display: flex; align-items: center; padding: 14px 18px; gap: 14px;">
				...
				<div style="flex: 1; min-width: 0;">
					<p style="margin: 0 0 4px; font-size: 13.5px; font-weight: 600; color: #1d2327;">
						<?php esc_html_e('Enjoying WPB Popup for Contact Form 7?', 'wpb-popup-for-contact-form-7'); ?>
					</p>
					<p style="margin: 0 0 11px; font-size: 13px; color: #50575e; line-height: 1.55;">
						<?php esc_html_e("You've been using the plugin for a week — thank you! If it's been helpful, a quick 5-star review would mean the world to us and help others discover it.", 'wpb-popup-for-contact-form-7'); ?>
					</p>
					<div style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap;">
						<a href="<?php echo $rate_url; ?>"
							...
							&#9733; <?php esc_html_e('You deserve it!', 'wpb-popup-for-contact-form-7'); ?>
						</a>
						<a href="<?php echo $later_url; ?>"
							...
							<?php esc_html_e('Maybe Later', 'wpb-popup-for-contact-form-7'); ?>
						</a>
						<a href="<?php echo $dismiss_url; ?>"
							...
							<?php esc_html_e('I Already Did', 'wpb-popup-for-contact-form-7'); ?>
						</a>
					</div>
				</div>
			</div>
...
<?php
	...
	{
		if (empty($_GET['wpb_pcf_review_action'])) {
			return;
		}

		$nonce  = ! empty($_GET['_wpnonce']) ? sanitize_text_field(wp_unslash($_GET['_wpnonce'])) : '';
		$action = sanitize_text_field(wp_unslash($_GET['wpb_pcf_review_action']));

		if (! wp_verify_nonce($nonce, self::NONCE_ACTION)) {
			die(esc_html__('Nonce Error!!!', 'wpb-popup-for-contact-form-7'));
		}
		...
		wp_safe_redirect(remove_query_arg(array('wpb_pcf_review_action', '_wpnonce')));
		exit;
	}
}
